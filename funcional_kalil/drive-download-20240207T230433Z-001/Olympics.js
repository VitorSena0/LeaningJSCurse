...
const medalhasPorPais = (listaDeAtletas, pais) => {
  // Filtra os atletas do país específico
...
const ranking10Paises = (listaDeAtletas) => {
  ...
  const paisesMedalhas = listaDeAtletas.reduce((acc, atleta) => {
    ...
    const index = acc.findIndex((element) => element.pais === atleta.nationality);
    
    // Atualiza as medalhas se o país já estiver no array, senão, adiciona um novo objeto
    if (index !== -1) {
      acc[index].medalhas += 0;
    } else {
      acc.push(medalhasPorPais(listaDeAtletas, atleta.nationality));
    }
...
const outraFuncaoQueManipulaJson = (jsonObject) => {
  ...
  const q5 = ranking10Paises(jsonObject)
...
// função que pega o resultado de uma função e escreve em um arquivo json
  const mostraResultado = (() => {
    const linhas = q5.map(objetos => JSON.stringify(objetos) + '\n')
